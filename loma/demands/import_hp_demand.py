...
from demands.household_count import parse_bus_numbers, hausnummer_split
...
def check_heat_pumps(buses, input_path):
    ...
    for idx, row in hp_df.iterrows():
        ...
        found=False
        for num in num_list:
            exact_matches = street_buses[street_buses.parsed_numbers.apply(lambda nums: num in nums)]
...
def add_heat_loads_to_network(n):
    ...
    yearly_profiles = pd.read_hdf("data/data_bundle/heat_yearly_profile.hdf", key="yearly_profile")
    ...
    heat_demand_cells = peta_heat.merge(census_cells, left_on='zensus_population_id', right_on = 'zensus_pop', how="left")
    heat_demand_cells = heat_demand_cells[heat_demand_cells.scenario=='status2019']
    ...
    load_profiles = pd.DataFrame(0.0, index=snapshots, columns=bus_with_cell.index)
    ...
    for bus_idx, row in bus_with_cell.iterrows():
        ...
        daily_candidates = daily_profiles.loc[daily_profiles["zensus_population_id"] == zensus_id]
        ...
        temp_air = pd.read_csv("data/data_bundle/wetterdaten_2011_Luft.csv").set_index("MESS_DATUM")
