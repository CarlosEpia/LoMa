...
def adjust_network_shape(n, export_path, mv_grid_id=35725, lv_grid_id=1):
    ...
    lines['type_info'] = n.lines.cable_type  ###ToDo check, if husum cable type match with edisgo requirements
    lines['kind'] = 'cable'
    ...
    loads['sector'] = loads['name'].apply(      ####toDo: distinguish heat pump loads
        lambda x: 'cts' if 'cts' in x.lower()
        else 'industrial' if 'ind' in x.lower()
        else 'residential')
    loads['number_households'] = 1
    loads['voltage_level'] = 'lv '     ###adjust if there are also mv_loads
    
    ### network
    mv_grid_geom = gpd.read_file('data/Input_files/MV_grid_district/husum_district.shp')
...
def export_timeseries(n, export_path):
    ...
    loads_ts.to_csv(os.path.join(export_path, "load_timeseries.csv"), index=True)
    
    #export p_max_pu for load_shedding_gens
    gens_ts = n.generators_t.p_max_pu
    gens_ts.to_csv(os.path.join(export_path, "gen_p_max_pu_timeseries.csv"), index=True)
